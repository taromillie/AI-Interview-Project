...
| 创建日期 | 2026-08-27 |
| 文档类型 | 架构设计文档 |
| 关联文档 | [需求文档](requirements.md) v1.0 |
| 项目定位 | 课程 / 毕业设计项目 |
...
│   ├── main.js
│   ├── App.vue
│   ├── router/                  # 路由（登录/诊断/面试/复盘/规划/题库）
│   ├── stores/                  # Pinia（用户、面试会话、Provider）
│   ├── api/                     # Axios 封装 + SSE 客户端
│   ├── views/
│   │   ├── Login.vue
│   │   ├── Dashboard.vue        # 首页：入口聚合
│   │   ├── ResumeDiagnosis.vue  # 简历×JD 诊断
...
| `positions` | id, name, direction, difficulty, skills_json, is_public, creator_id, status | 岗位，方向可配置 |
...
| `interviews` | id, user_id, position_id, resume_id, mode(text/voice/video), interview_type(normal/switch/salary), status, config_json, created_at, finished_at | 面试会话 |
...
| 面试 | POST | `/api/interviews` | 创建面试会话 | JWT |
...
| 阶段 | 架构扩展 |
|---|---|
| Phase 1 MVP | 本文档全量实现（SQLite + ChromaDB + 文字/语音面试） |
| Phase 2 | 能力画像/成长追踪/转行诊断/谈薪模块（复用编排器） |
| Phase 3 | 公共题库审核、视频面试（face-api.js） |
| Phase 4 | PostgreSQL + Qdrant 生产化、多岗位方向扩展、离线 RAG 评测 |
