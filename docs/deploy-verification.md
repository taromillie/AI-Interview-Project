...
预期：构建后端（Python 3.14 + requirements）与前端（node:20-alpine 构建 + nginx:alpine 托管）两个镜像并启动。
...
| 5 | 数据库持久化 | 登录后：`docker compose restart backend`，刷新页面 | 会话/数据不丢失（volume `sqlite_data` 持久化 `app.db`） |
...
|--------|----------------|---------------------|
| Python 环境 | 本机 `.venv`（3.14） | 镜像内 Python 3.14（requirements 安装） |
...
| 网络 | localhost 直连 | 容器网络 + Nginx 反代 `/api` `/sse` |
